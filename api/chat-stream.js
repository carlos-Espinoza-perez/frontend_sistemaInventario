...
class IA {
  ...
  async assistant_streamMode() {
    ...
    const runStream = openai.beta.threads.runs
      .createAndStream(this.threadId, { assistant_id: assistantID })
      .on("textDelta", (delta) => {
        this.res.write(encoder.encode(`data: ${JSON.stringify({ text: delta.value })}\n\n`));
      });
  ...
  async normal_streamMode() {
    try {
      ...
      for await (const chunk of stream) {
        ...
        if (delta) {
          this.res.write(encoder.encode(`data: ${JSON.stringify({ text: delta })}\n\n`));
        }
      }
