  ...
  return (
    <div className="flex items-center gap-4 bg-white p-4 rounded-xl shadow min-h-[80px]">
      ...
      <div className="flex-1 flex flex-col justify-center">
        ...
        <p className="text-[var(--text-secondary)] text-xs line-clamp-1">Precio de compra: {formatMoney(item.purchase_price)}</p>
      ...
      <div className="text-right">
        ...
        <p className="text-[var(--success-color)] text-sm font-medium"> {formatMoney((item.sale_price - item.purchase_price) * item.quantity)} </p>
      </div>
    </div>
  );
}
